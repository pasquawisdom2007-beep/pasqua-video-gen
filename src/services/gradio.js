...
async function connectWithRetry(spaceName, maxRetries = 3) {
  ...
  // Falls back to a hardcoded token if the Render env var isn't picked up.
  // Since this repo is private, the user has accepted the risk of keeping
  // a real token in source rather than relying solely on Render's dashboard.
  const token = process.env.HF_TOKEN || "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";
